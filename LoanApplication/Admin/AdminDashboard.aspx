...
<head runat="server">
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .sidebar a { color: #fff; text-decoration: none; padding: 12px 15px; border-radius: 8px; margin-bottom: 10px; display: block; transition: 0.3s; }
...
<body>
    ...
    <div class="main-content">
                ...
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    try
                    {
                        ...
                        if (action == "approve")
                            ...
                            query = "INSERT INTO DeletedApplications (ApplicationID, FullName, Email, Phone, Address, LoanType, LoanAmount, EmploymentType, MonthlyIncome, Status, AppliedAt) " +
                                    "SELECT ApplicationID, FullName, Email, Phone, Address, LoanType, LoanAmount, EmploymentType, MonthlyIncome, Status, AppliedAt FROM LoanApplications WHERE ApplicationID=@ID; " +
                                    "DELETE FROM LoanApplications WHERE ApplicationID=@ID;";
                        else if (action == "updateStatus")
                        {
                            string newStatus = Request.Form["newStatus"];
                            query = "UPDATE LoanApplications SET Status=@NewStatus WHERE ApplicationID=@ID";
                            MySqlCommand cmdUpdate = new MySqlCommand(query, conn);
                            cmdUpdate.Parameters.AddWithValue("@ID", applicationId);
                            cmdUpdate.Parameters.AddWithValue("@NewStatus", newStatus);
                            cmdUpdate.ExecuteNonQuery();
                            alertMessage = "✓ Status updated to " + newStatus + "!";
                            alertType = "success";
                            query = ""; // prevent re-execution
                        }
                        ...
                        if (!string.IsNullOrEmpty(query))
                        {
                            ...
                            if (action == "approve") { alertMessage = "✓ Application approved!"; alertType = "success"; }
                            else if (action == "reject") { alertMessage = "✓ Application rejected!"; alertType = "success"; }
                            else if (action == "delete") { alertMessage = "✓ Application moved to Trash!"; alertType = "success"; }
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        <div class="stats-container">
            <a href="AdminDashboard.aspx" class="stat-card"><div class="stat-number"><%= totalApps %></div><div class="stat-label">Total Applications</div></a>
            <a href="AdminDashboard.aspx?filterStatus=Pending" class="stat-card pending"><div class="stat-number"><%= pendingCount %></div><div class="stat-label">Pending</div></a>
            <a href="AdminDashboard.aspx?filterStatus=Approved" class="stat-card approved"><div class="stat-number"><%= approvedCount %></div><div class="stat-label">Approved</div></a>
            <a href="AdminDashboard.aspx?filterStatus=Rejected" class="stat-card rejected"><div class="stat-number"><%= rejectedCount %></div><div class="stat-label">Rejected</div></a>
        </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>ID</th><th>Full Name</th><th>Email</th><th>Phone</th><th>Loan Type</th>
                        <th>Amount</th><th>Employment</th><th>Income</th><th>Status</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <%
                    using (MySqlConnection conn2 = new MySqlConnection(connString))
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(searchName)) query += " AND FullName LIKE @SearchName";
                            if (!string.IsNullOrEmpty(filterStatus)) query += " AND Status=@FilterStatus";
                            if (!string.IsNullOrEmpty(filterLoanType)) query += " AND LoanType=@FilterLoanType";
                            ...
                            if (!string.IsNullOrEmpty(filterStatus)) cmd.Parameters.AddWithValue("@FilterStatus", filterStatus);
                            if (!string.IsNullOrEmpty(filterLoanType)) cmd.Parameters.AddWithValue("@FilterLoanType", filterLoanType);
                            ...
                            while (reader.Read())
                            {
                                ...
                                string statusClass = status=="Pending"?"status-pending":status=="Approved"?"status-approved":"status-rejected";
                                ...
                                Response.Write("<form method='post' style='display:inline;'>");
                                Response.Write("<input type='hidden' name='applicationId' value='" + reader["ApplicationID"] + "' />");
                                Response.Write("<select name='newStatus' style='padding:4px 6px; border-radius:4px; font-size:12px;'>");
                                string[] statuses = new string[] { "Pending", "Approved", "Rejected" };
                                foreach (string s in statuses)
                                    if (s != status) Response.Write("<option value='" + s + "'>" + s + "</option>");
                                Response.Write("</select>");
                                Response.Write("<button type='submit' name='action' value='updateStatus' class='btn-action' style='background:#007bff;'>Update</button>");
                                Response.Write("</form>");
